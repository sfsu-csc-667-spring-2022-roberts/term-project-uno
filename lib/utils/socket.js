...
async function handleReverse(game) {
  if (! await GamesDao.updateReversed(!game.playerOrderReversed, game.id)) return { fail: true }
...
async function handlePlus2(game) {
  ...
  return { amount, playerThatDrawsIndex, skip: true, refreshDrawCards: result.update }
}
async function handlePlus4Choose(game, chosenColor) {
  ...
  return { amount, playerThatDrawsIndex, newColor: updatingColor.newColor, skip: true, refreshDrawCards: result.update }
}
...
async function handleSwap(game, player1, player2username, card) {
  ...
  const players = await PlayersDao.findPlayers(user2.id);

  if (!players) return { fail: true }

  let p1cards = await PlayerCardsDao.findPlayerCardIdsAndDelete(player1.id);
  let p2cards = await PlayerCardsDao.findPlayerCardIdsAndDelete(players[0].id);

  await PlayedCardsDao.createPlayedCard(card.id, game.id);

  p1cards.filter(p1card => p1card.cardId !== card.id).forEach(p1card => PlayerCardsDao.createPlayerCard(p1card.cardId, players[0].id))
...
async function emitBasedOnCardType(game, player, card, io, chosenColor, player2username) {
  ...
  if (cardCount === 0) {
    // only edge case that is not covered is when the last card is a swap hands
  ...
  for (let i = 0; i < amount; i++) {
    ...
    if (count === 0) {
      ...
      while (playedDeck.length > 0) {
        ...
        await PlayedCardsDao.removePlayedCard(game.id, playedDeck[index].cardId)
        await DrawCardsDao.createDrawCard(playedDeck[index].cardId, game.id);
...
async function emitDrawCards(game, player, io) {
  ...
  if (result.update) newGameState.refreshDrawCards = true;
