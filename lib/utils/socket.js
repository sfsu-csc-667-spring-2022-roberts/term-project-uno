...
async function handleSwap(game, player1, player2username, card) {
  ...
  const player = await PlayersDao.findPlayer(user2.id, game.id);
...
async function emitBasedOnCardType(game, player, card, io, chosenColor, player2username) {
  try {
    ...
    await refillDrawCards(game.id, ret);

    if (ret.update === true) {
      newGameState.refreshDrawCards = true;
      newGameState.drawDeckCount = ret.drawDeckCount;
    }
...
async function refillDrawCards(gameId, ret) {
  const count = await DrawCardsDao.findCardCount(gameId);
  const playedCardsCount = await PlayedCardsDao.findCardCount(gameId);
  if (count === 0 && playedCardsCount > 25) {
    let playedDeck = await PlayedCardsDao.findAllPlayedCards(gameId);
    ...
    while (playedDeck.length > 0) {
      ...
      await PlayedCardsDao.removePlayedCard(gameId, playedDeck[index].id)
      await DrawCardsDao.createDrawCard(playedDeck[index].id, gameId);
      playedDeck = playedDeck.filter((r,i) => i !== index);
    }
    const drawDeckCount = await DrawCardsDao.findCardCount(gameId);
    ret.update = true;
...
async function emitDrawCards(game, player, io) {
  ...
  try {
    ...
    if (result === -2 && newTurnIndex != -1) {
      io.to(`game/${game.id}`).emit('draw-card', {
        ...
        amount: 1,
        refreshDrawCards: true,
        drawDeckCount: 0,
      });
    }
    else if (result.retval !== 1 || newTurnIndex === -1) return;
